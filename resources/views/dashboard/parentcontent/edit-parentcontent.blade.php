...
<div>
    <div class="grid-form1">
        <h3 id="forms-horizontal"> Edit Parent Content Page</h3>

        <a href="{{ url('/all-parentcontent') }}" class="btn btn-primary float-end"> view all Parent Contents </a>

        @if(Session::has('parentcontent_updated'))
        <p class="alert alert-success"> {{ Session::get('parentcontent_updated') }}</p>
        @endif
        <form class="form-horizontal" action="{{ route('parentcontent.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <select name="parentpage_id" id="parentpage_id">
                        @foreach ($parentpages as $parentpage )
                        <option value="{{ $parentpage->id }}" {{ $parentpage->parentpage_id == $parentpage->id ? 'selected' : '' }}> {{ $parentpage->title }} </option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                <div>
                    <label for="for description" class="col-sm-2 control-label hor-form">Description:</label>
                </div>
                <div class="col-sm-10">
